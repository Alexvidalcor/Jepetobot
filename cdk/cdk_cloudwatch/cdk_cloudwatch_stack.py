...
class CloudWatchStack(Stack):
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        logGroup = logs.LogGroup(self, "{appName}-{envDeploy}-log_group",
            log_group_name=f"{appName}-{envDeploy}-log_group",
        ...
        for logsFilename in listLogsPath:
            logStream = logs.LogStream(
                self,
                f'{appName}-{envDeploy}-{logsFilename.split("/")[-1]}-stream',
                log_group=logGroup,
                log_stream_name=f'{appName}-{envDeploy}-{logsFilename.split("/")[-1]}-stream'
            )
